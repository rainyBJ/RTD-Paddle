...
class VideoRecord:
    ...
    def __init__(self, vid, num_frames, locations, gt, s_e_scores, fps, args):
        ...
        self.locations_norm = [((i - self.base) / (self.window_size * self.
            interval)) for i in locations]
        self.locations_offset = [(location - self.base) for location in
            locations]
        ...
        if args.point_prob_normalize is True:
            ...
            s_e_scores[:, 0] = (s_e_scores[:, 0] - np.min(s_e_scores[:, 0])
                ) / range_start
            s_e_scores[:, 1] = (s_e_scores[:, 1] - np.min(s_e_scores[:, 1])
                ) / range_end
        self.s_e_scores = s_e_scores


class ThumosDetection(paddle.io.Dataset):
    ...
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for vid in video_list:
            if self.split in vid:
                ...
                annotations = [item['segment_frame'] for item in self.
                    anno_file[vid]['annotations']]
                labels = [int(item['label']) for item in self.anno_file[vid
                    ]['annotations']]
                s_e_seq = pd.read_csv(os.path.join(self.tem_folder, vid +
                    '.csv'))
                ...
                if seq_len <= self.window_size:
                    ...
                    gt = [(annotations[idx], labels[idx]) for idx in range(
                        len(annotations))]
                    self.video_list.append(VideoRecord(vid, num_frames,
                        locations, gt, s_e_scores, fps, args))
                else:
                    if self.split == 'test':
                        overlap_ratio = 2
                    else:
                    ...
                    ws_starts = [(i * stride) for i in range((seq_len //
                        self.window_size - 1) * overlap_ratio + 1)]
                    ...
                    for ws in ws_starts:
                        ...
                        for idx in range(len(annotations)):
                            ...
                            label = labels[idx]
                            if anno[0] >= locations[0] and anno[1
                                ] <= locations[-1]:
                                gt.append((anno, label))
                        if self.split == 'test':
                            self.video_list.append(VideoRecord(vid,
                                num_frames, locations, gt, s_e_scores, fps,
                                args))
                        elif len(gt) > 0:
                            self.video_list.append(VideoRecord(vid,
                                num_frames, locations, gt, s_e_scores, fps,
    ...
    def get_data(self, video: VideoRecord):
        ...
        base = video.base
        og_locations = torch2paddle.create_tensor([location for location in
            video.locations])
        vid_feature = paddle.load(os.path.join(self.feature_folder, vid))
        ft_idxes = [min(i // 8, vid_feature.shape[0] - 1) for i in og_locations
            ]
        snippet_fts = []
        for i in ft_idxes:
            i = int(i)
            snippet_fts.append(vid_feature[i].squeeze())
        snippet_fts = paddle.stack(snippet_fts)
        assert snippet_fts.shape == [self.window_size, 2048], print(snippet_fts
            .shape)
        if video.absolute_position:
            locations = torch2paddle.create_tensor([location for location in
                video.locations])
        else:
            locations = torch2paddle.create_tensor([location for location in
                video.locations_offset])
        s_e_scores = torch2paddle.create_tensor(video.s_e_scores)
        ...
        targets = {'labels': [], 'boxes': [], 'video_id': torch2paddle.
            create_tensor([self.video_dict[vid]])}
        for start, end, label in gt_s_e_frames:
            targets['labels'].append(int(label))
            targets['boxes'].append((start, end))
        targets['labels'] = LongTensor(targets['labels'])
        targets['boxes'] = torch2paddle.create_tensor(targets['boxes'])
        return (vid, locations, snippet_fts, targets, num_frames, base,
            s_e_scores)
...
def collate_fn(batch):
    vid_name_list, target_list, num_frames_list, base_list = [[] for _ in
        range(4)]
    batch_size = len(batch)
    ft_dim = batch[0][2].shape[-1]
    max_props_num = batch[0][1].shape[0]
    snippet_fts = paddle.zeros([batch_size, max_props_num, ft_dim]
        ).requires_grad_(False)
    locations = paddle.zeros([batch_size, max_props_num, 1], dtype=paddle.
        float64).requires_grad_(False)
    s_e_scores = paddle.zeros([batch_size, max_props_num, 2]).requires_grad_(
        False)
    for i, sample in enumerate(batch):
        vid_name_list.append(sample[0])
        target_list.append(sample[3])
        snippet_fts[i, :max_props_num, :] = sample[2]
        locations[i, :max_props_num, :] = sample[1].reshape(-1, 1).cast(dtype="float64")
        num_frames_list.append(sample[4])
        if sample[5] is not None:
            base_list.append(sample[5])
        s_e_scores[i, :max_props_num, :] = sample[6].cast(dtype="float32")
    num_frames = paddle.to_tensor(np.array(num_frames_list))
    base = paddle.to_tensor(np.array(base_list))
...
def get_args_parser():
    ...
    parser.add_argument('--feature_path', default='/data1/tj/thumos_2048/',
        type=str)
    parser.add_argument('--tem_path', default=\
        '/data1/tj/BSN_share/output/TEM_results', type=str)
    parser.add_argument('--annotation_path', default=\
        'thumos14_anno_action.json', type=str)
