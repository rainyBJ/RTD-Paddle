...
def train_one_epoch(model: paddle.nn.Layer, criterion: paddle.nn.Layer,
    data_loader: Iterable, optimizer: paddle.optimizer.Optimizer, device:
    paddle.set_device, epoch: int, args, postprocessors=None):
    model.train()
    criterion.train()
    ...
    for vid_name_list, locations, samples, targets, num_frames, base, s_e_scores in metric_logger.log_every(
        ...
        loss_dict_reduced_scaled = {k: (v * weight_dict[k]) for k, v in
            loss_dict_reduced.items() if k in weight_dict}
        ...
        losses.backward()
        if max_norm > 0:
            paddle.fluid.layers.nn.clip_by_norm(model.parameters(), max_norm)
        optimizer.step()
