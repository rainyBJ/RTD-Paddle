...
def get_args_parser():
    ...
    parser.add_argument('--nheads', default=8, type=int, help=\
        "Number of attention heads inside the transformer's attentions")
    parser.add_argument('--num_queries', default=100, type=int, help=\
        'Number of query slots')
    parser.add_argument('--pre_norm', action='store_true')
    parser.add_argument('--no_aux_loss', dest='aux_loss', action=\
        'store_false', help=\
        'Disables auxiliary decoding losses (loss at each layer)')
    parser.add_argument('--set_cost_class', default=1, type=float, help=\
        'Class coefficient in the matching cost')
    parser.add_argument('--set_cost_bbox', default=5, type=float, help=\
        'L1 box coefficient in the matching cost')
    parser.add_argument('--set_cost_giou', default=2, type=float, help=\
    ...
    parser.add_argument('--eos_coef', default=0.1, type=float, help=\
    ...
    parser.add_argument('--annotation_path', default=\
        'datasets/thumos14_anno_action.json', type=str)
    ...
    parser.add_argument('--stage', default=1, type=int, help=\
        'stage-id of RTD-Net')
    parser.add_argument('--output_dir', default='outputs/', help=\
...
def main(args):
    ...
    if args.stage == 1:
        ...
        for name, value in model_without_ddp.named_parameters():
            if 'class_embed' not in name:
                value.requires_grad = False
        head_params = filter(lambda p: p.requires_grad, model_without_ddp.
            parameters())
        ...
        head_params = filter(lambda p: p.requires_grad, model_without_ddp.
            parameters())
        learned_params = list(head_params)
    optimizer = paddle.optimizer.AdamW(learning_rate=args.lr, parameters=learned_params, weight_decay=args.weight_decay)
    lr_scheduler = paddle.optimizer.lr.StepDecay(learning_rate=args.lr, step_size=args.lr_drop)
    dataset_train = build_dataset(image_set='train', args=args)
    dataset_val = build_dataset(image_set='val', args=args)
    if args.distributed:
        sampler_train = paddle.io.DistributedBatchSampler(dataset=dataset_train, batch_size=dataset_train.__len__())
        sampler_val = paddle.io.DistributedBatchSampler(dataset=dataset_val, batch_size=dataset_val.__len__(), shuffle=False)
    else:
    ...
    data_loader_train = DataLoader(dataset_train, batch_sampler=\
        batch_sampler_train, collate_fn=thumos.collate_fn, num_workers=args
        .num_workers)
    data_loader_val = DataLoader(dataset_val, args.batch_size, sampler=\
        sampler_val, drop_last=False, collate_fn=thumos.collate_fn,
    ...
    if args.eval:
        evaluator, eval_loss_dict = evaluate(model, criterion,
            postprocessors, data_loader_val, device, args)
    ...
    axs_train = {'loss_ce': ax1_train, 'loss_bbox': ax2_train, 'loss_giou':
        ax3_train, 'cardinality_error': ax4_train, 'class_error': ax5_train,
        'loss_iou': ax6_train}
    ...
    axs_eval = {'loss_ce': ax1_eval, 'loss_bbox': ax2_eval, 'loss_giou':
        ax3_eval, 'cardinality_error': ax4_eval, 'class_error': ax5_eval,
        'loss_iou': ax6_eval}
    colordict = {'50': 'g', '100': 'b', '200': 'purple', '500': 'orange',
        '1000': 'brown'}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats, train_loss_dict = train_one_epoch(model, criterion,
            data_loader_train, optimizer, device, epoch, args)
        for key, value in train_loss_dict.items():
            if key in ['loss_ce', 'loss_bbox', 'loss_giou',
                'cardinality_error', 'class_error', 'loss_iou']:
                try:
        ...
        if epoch % 50 == 0 and args.output_dir:
            checkpoint_path = (output_dir / 'checkpoint_epoch{}.pdiparams'.
                format(epoch))
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
            **{f'test_AR@{k}': v for k, v in test_stats.items()}, 'epoch':
        ...
        if float(test_stats['50']) > best_ar50:
            ...
            checkpoint_path = output_dir / 'checkpoint_best_ar50.pdiparams'
            utils.save_on_master({'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(), 'lr_scheduler':
                lr_scheduler.state_dict(), 'epoch': epoch, 'args': args},
                checkpoint_path)
        current_sum_ar = float(test_stats['50']) + float(test_stats['100']
            ) + float(test_stats['200'])
        if current_sum_ar > best_sum_ar:
            best_sum_ar = current_sum_ar
            with (output_dir / 'log_best_sum_ar.txt').open('w') as f:
                f.write(json.dumps(log_stats) + '\n')
            checkpoint_path = output_dir / 'checkpoint_best_sum_ar.pdiparams'
            utils.save_on_master({'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(), 'lr_scheduler':
                lr_scheduler.state_dict(), 'epoch': epoch, 'args': args},
                checkpoint_path)
