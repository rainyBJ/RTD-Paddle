...
class PositionEmbeddingSine(nn.Layer):
    ...
    def _get_position_angle_vector(self, position):
        return [(position / np.power(100, 2 * (i // 2) / self.hidden_dim)) for
            i in range(self.hidden_dim)]

    def _get_encoding_table(self):
        table = np.array([self._get_position_angle_vector(position) for
            position in range(self.num_locations)])
...
def build_position_embedding(args):
    if args.position_embedding in ('v2', 'sine'):
        position_embedding = PositionEmbeddingSine(hidden_dim=args.
            hidden_dim, num_locations=args.window_size)
    elif args.position_embedding in ('v3', 'learned'):
        position_embedding = PositionEmbeddingLearned(hidden_dim=args.
            hidden_dim, num_locations=args.window_size)
    else:
...
def get_args_parser():
    ...
    parser.add_argument('--hidden_dim', default=256, type=int, help=\
