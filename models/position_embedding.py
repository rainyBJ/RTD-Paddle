...
class PositionEmbeddingSine(nn.Layer):
    ...
    def _get_encoding_table(self):
        table = np.array([self._get_position_angle_vector(position)
                          for position in range(self.num_locations)]).astype("float32")
        table[:, 0::2] = np.sin(table[:, 0::2])
        table[:, 1::2] = np.cos(table[:, 1::2])
        return paddle.to_tensor(table).unsqueeze(0)

    def forward(self, locations):
        result = self.position_table[:locations.shape[1]].clone().detach().tile([locations.shape[0], 1, 1])
        return result
...
class PositionEmbeddingLearned(nn.Layer):
    ...
    def forward(self, locations):
        i = paddle.arange(locations.shape[1])
...
if __name__ == '__main__':
    ...
    locations = paddle.ones([1, args.window_size, 1])
    locations[0, :, 0] = paddle.arange(args.window_size)
