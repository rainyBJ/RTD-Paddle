...
class RTD(nn.Layer):
    ...
    def __init__(self, position_embedding, transformer, num_classes,
        num_queries, stage, aux_loss=False):
        super().__init__()
        ...
        self.iou_conv = nn.Sequential(nn.Conv1D(self.hidden_dim, self.
            hidden_dim * 2, kernel_size=3, padding=1), x2paddle.
            torch2paddle.ReLU(inplace=True), nn.Conv1D(self.hidden_dim * 2,
            self.hidden_dim, kernel_size=3, padding=1))
    ...
    def forward(self, locations, samples, s_e_scores):
        ...
        features = torch2paddle.concat((features_s, features_e), dim=2)
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_iou):
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_iou': c} for a, b,
            c in zip(outputs_class[:-1], outputs_coord[:-1], outputs_iou[:-1])]
...
class SetCriterion(nn.Layer):
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        card_err = paddle.nn.functional.l1_loss(card_pred.float(),
            tgt_lengths.float())
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss_bbox = paddle.nn.functional.l1_loss(src_boxes, box_ops.
            prop_se_to_cl(target_boxes), reduction='none')
        losses = {}
        losses['loss_bbox'] = loss_bbox.sum() / num_boxes
        loss_giou = 1 - paddle.diag(box_ops.generalized_prop_iou(box_ops.
            prop_cl_to_se(src_boxes), target_boxes))
    ...
    def loss_iou(self, outputs, targets, indices, num_boxes):
        ...
        iou_loss = torch2paddle.sum(iou_loss * iou_mask) / (1e-06 +
            torch2paddle.sum(iou_mask)).float()
        losses = {'loss_iou': iou_loss}
        return losses

    def _get_src_permutation_idx(self, indices):
        batch_idx = torch2paddle.concat([paddle.full_like(src, i).
            requires_grad_(False) for i, (src, _) in enumerate(indices)])
        src_idx = torch2paddle.concat([src for src, _ in indices])
        return batch_idx, src_idx

    def _get_tgt_permutation_idx(self, indices):
        batch_idx = torch2paddle.concat([paddle.full_like(tgt, i).
            requires_grad_(False) for i, (_, tgt) in enumerate(indices)])
        tgt_idx = torch2paddle.concat([tgt for _, tgt in indices])
        return batch_idx, tgt_idx

    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {'labels': self.loss_labels, 'cardinality': self.
            loss_cardinality, 'boxes': self.loss_boxes, 'iou': self.loss_iou}
...
class PostProcess(nn.Layer):
    ...
    def forward(self, outputs, num_frames, base):
        ...
        if not self.absolute_position:
            boxes = box_ops.prop_relative_to_absolute(boxes, base, self.
                window_size, self.interval)
        else:
            bs, ws, _ = boxes.shape
            scale_fct = num_frames.unsqueeze(-1).repeat((1, ws, 2)).to(boxes
                .device)
            boxes = boxes * scale_fct
        if self.stage != 3:
            results = [{'scores': s, 'labels': l, 'boxes': b, 'iou_score':
                i} for s, i, l, b in zip(scores, out_iou, labels, boxes)]
        if self.stage == 3:
            new_scores = 0.5 * (scores.squeeze() + out_iou.squeeze())
            results = [{'scores': s, 'labels': l, 'boxes': b, 'iou_score':
                i} for s, i, l, b in zip(new_scores, out_iou, labels, boxes)]
        return results
...
def build(args):
    ...
    model = RTD(position_embedding=position_embedding, transformer=\
        transformer, num_classes=num_classes, num_queries=args.num_queries,
        stage=args.stage, aux_loss=args.aux_loss)
    ...
    if args.stage != 3:
        ...
        losses = ['iou']
    criterion = SetCriterion(num_classes, matcher=matcher, weight_dict=\
        weight_dict, eos_coef=args.eos_coef, losses=losses)
    criterion.to(device)
