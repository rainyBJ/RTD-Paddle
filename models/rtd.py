...
class RTD(nn.Layer):
    ...
    def forward(self, locations, samples, s_e_scores):
        ...
        projected_fts = projected_fts.reshape([bs, -1, self.hidden_dim // 2])
        scaling_factor = 2
        ...
        proposal_fts = hs[-1, :, :, :].transpose([0, 2, 1])
        proposal_fts = self.iou_conv(proposal_fts)
        proposal_fts = proposal_fts.transpose([0, 2, 1])
...
class SetCriterion(nn.Layer):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        zip_file = zip(targets, indices)
        list1 = []
        for t,(_,j) in zip_file:
            if j.shape==[0]:
                continue
            item = t['labels'][j]
            list1.append(item)
        # foo = [t['labels'][J] for t, (_, J) in zip(targets, indices)]
        target_classes_o = paddle.concat(list1)
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        tgt_lengths = paddle.to_tensor(data=[len(v['labels']) for v in targets],
                                       place=device)
        card_pred = (pred_logits.argmax(-1) != pred_logits.shape[-1] - 1).astype("int64").sum(1)
        card_err = paddle.nn.functional.l1_loss(card_pred.astype("float32"), tgt_lengths.astype("float32"))
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        for t, (_, j) in zip_file:
            if j.shape == [0]:
                continue
            item = t['boxes'][j].reshape([-1,t['boxes'][j].shape[-1]])
            list1.append(item)
        # list_concat = [t['boxes'][i].reshape([-1,t['boxes'][i].shape[-1]]) for t, (_, i) in zip(targets, indices)]
        target_boxes = paddle.concat(list1, axis=0)
        ...
        inter1 = box_ops.prop_cl_to_se(src_boxes)
        inter2 = box_ops.generalized_prop_iou(inter1, target_boxes)
        # inter3 = paddle.diag(inter2)
        # diag
        list1 = []
        for i in range(inter2.shape[0]):
            list1.append(inter2[i, i])
        inter3 = paddle.concat(list1)
        loss_giou = 1 - inter3
        # loss_giou = paddle.abs(inter1-target_boxes)
    ...
    def loss_iou(self, outputs, targets, indices, num_boxes):
        ...
        for i in range(bs):
            ...
            if len(target_boxes_per_seg) == 0:
                tiou = paddle.zeros(
                    [len(pred_boxes_per_seg)])
                # tiou = paddle.zeros(
                #     [len(pred_boxes_per_seg)]).to(pred_boxes_per_seg.place)
            else:
        ...
        tgt_iou = paddle.stack(tgt_iou, axis=0).reshape([-1])
        preds_iou = preds_iou.reshape([-1])
        ...
        iou_mask = (tgt_iou > 0.7).astype("float32")
        iou_mask[sampled_m_ind] = 1.0
        iou_mask[sampled_neg_ind] = 1.0
        iou_loss = F.smooth_l1_loss(preds_iou, tgt_iou.squeeze()).astype("float32")
        iou_loss = paddle.sum(iou_loss * iou_mask) / (1e-06 + paddle.sum(iou_mask)).astype("float32")
    ...
    def forward(self, outputs, targets):
        ...
        indices = self.matcher(outputs_without_aux, targets) # 慢
...
class PostProcess(nn.Layer):
    ...
    def forward(self, outputs, num_frames, base):
        ...
        foo = prob[..., :-1]
        scores = foo.max(-1)
        labels = paddle.zeros_like(scores)
        ...
        if not self.absolute_position:
            ...
            bs, ws, _ = boxes.shape
            scale_fct = num_frames.unsqueeze(-1).tile((1, ws, 2)).to(boxes.device)
