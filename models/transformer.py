...
class Transformer(nn.Layer):
    ...
    def forward(self, src, query_embed, pos_embed):
        ...
        pos_embed = pos_embed.transpose([1, 0, 2])
        query_embed = query_embed.unsqueeze(1).tile([1, bs, 1])

        tgt = paddle.full_like(query_embed, fill_value=0.0)

        src = paddle.concat([src, pos_embed], axis=2)
        memory = self.encoder(src)
        hs = self.decoder(tgt.transpose([1, 0, 2]),
                          memory.transpose([1, 0, 2]),
                          memory_key_padding_mask=None,
                          pos=pos_embed.transpose([1, 0, 2]),
                          query_pos=query_embed.transpose([1, 0, 2]))
        hs_ = paddle.unsqueeze(paddle.tile(memory[1],[32,1,1]),0)
        return hs, memory.transpose([1, 2, 0]).reshape([bs, c, t])
