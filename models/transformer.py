...
class Transformer(nn.Layer):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=3,
        ...
        ='relu', normalize_before=False, return_intermediate_dec=False):
        super().__init__()
...
class TransformerDecoder(nn.Layer):
    ...
    def forward(self, tgt, memory, tgt_mask: Optional[create_tensor]=None,
        memory_mask: Optional[create_tensor]=None, tgt_key_padding_mask: Optional[
        create_tensor]=None, memory_key_padding_mask: Optional[create_tensor]=None, pos:
        Optional[create_tensor]=None, query_pos: Optional[create_tensor]=None):
        ...
        for layer in self.layers:
            output = layer(output, memory, tgt_mask=tgt_mask, memory_mask=\
                memory_mask, tgt_key_padding_mask=tgt_key_padding_mask,
...
class TransformerDecoderLayer(nn.Layer):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.self_attn = paddle.nn.MultiHeadAttention(d_model, nhead,
            dropout=dropout)
        self.multihead_attn = paddle.nn.MultiHeadAttention(d_model, nhead,
            dropout=dropout)
    ...
    def forward_post(self, tgt, memory, tgt_mask: Optional[create_tensor]=None,
        memory_mask: Optional[create_tensor]=None, tgt_key_padding_mask: Optional[
        create_tensor]=None, memory_key_padding_mask: Optional[create_tensor]=None, pos:
        Optional[create_tensor]=None, query_pos: Optional[create_tensor]=None):
    ...
    def forward_pre(self, tgt, memory, tgt_mask: Optional[create_tensor]=None,
        memory_mask: Optional[create_tensor]=None, tgt_key_padding_mask: Optional[
        create_tensor]=None, memory_key_padding_mask: Optional[create_tensor]=None, pos:
        Optional[create_tensor]=None, query_pos: Optional[create_tensor]=None):
        """Forward of decoder layer in transformer, layer normalization
...
def build_transformer(args):
    return Transformer(d_model=args.hidden_dim, dropout=args.dropout, nhead
        =args.nheads, dim_feedforward=args.dim_feedforward,
        num_encoder_layers=args.enc_layers, num_decoder_layers=args.
        dec_layers, normalize_before=args.pre_norm, return_intermediate_dec
        =True)
