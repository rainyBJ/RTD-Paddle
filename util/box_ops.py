...
def prop_relative_to_absolute(x, base, window_size, interval):
    ...
    base = base.unsqueeze(1).tile([1, num_samples]).astype("float32")
...
#
#     list = []
#     # tiou = paddle.zeros((N, M))
#     # if box_a.stop_gradient:
#     #     tiou.stop_gradient = True
#     # else:
#     #     tiou.stop_gradient = False
#     for i in range(N):
#         foo0 = box_a[i, 1].tile([M]).unsqueeze([0])
#         foo1 = box_b[:, 1].unsqueeze([0])
#         foo2 = paddle.concat([foo0, foo1])
#         inter_max_xy = paddle.min(foo2,axis=0)
#         foo0 = box_a[i, 0].tile([M]).unsqueeze([0])
#         foo1 = box_b[:, 0].unsqueeze([0])
#         foo2 = paddle.concat([foo0, foo1])
#         inter_min_xy = paddle.max(foo2,axis=0)
#         inter = paddle.clip(inter_max_xy - inter_min_xy, min=0)
#         union = box_b[:, 1] - box_b[:, 0] + (box_a[i, 1] - box_a[i, 0]) - inter
#         item = inter/union
#         list.append(paddle.unsqueeze(item,0))
#         # tiou[i:] = inter/union
#     tiou = paddle.concat(list)
#
#     return tiou  # (N, M)
def segment_tiou(box_a, box_b):
    N = box_a.shape[0]
    ...
    for i in range(N):
        ...
        inter = paddle.clip(inter_max_xy - inter_min_xy, min=0)
        union = box_b[:, 1] - box_b[:, 0] + (box_a[i, 1] - box_a[i, 0]) - inter
        list_to_add.append(inter / union)
    tiou = paddle.stack(list_to_add, axis=0)
